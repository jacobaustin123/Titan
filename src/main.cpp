...
int main( void )
{
    Scene s(1000); // initialize scene object
    ...
    std::vector<GLubyte> indices; // this contians the order in which to draw the lines between points
    for (int i = 0; i < 7; i++) {
        for (int j = i + 1; j < 8; j++) {
            indices.push_back(i);
            indices.push_back(j);
    ...
    do{
        ...
        for (int i = 0; i < 8; i++) { // populate buffer with position data for cube vertices
            g_vertex_buffer_data[3*i] = s.cube.masses[i].P[0];
            g_vertex_buffer_data[3*i + 1] = s.cube.masses[i].P[1];
            g_vertex_buffer_data[3*i + 2] = s.cube.masses[i].P[2];
        }
        
        s.next(); // update the physics
        count++; // iterate count
        
        if (count % 1000 == 0) // print some random information every 1000 iterations
            std::cout << count << ": " << s.cube.masses[5].P << std::endl;
